...
class SubmissionSettings:
    ...
    service_owner_id: str = _env("CFG_OWNER_SERVICE_ID", applicant_user_id)
...
def _parse_install_items(text: str, plan_type: str) -> List[Dict[str, Any]]:
    ...
    for token in tokens:
        ...
        if not mapped_list:
            mapped_list = [{"name": name, "code": None, "cycle": None}]
...
def _auto_create_tasks_for_opportunity(
    ...
    payload = {
        "data": {
            ...
            "originator": settings.service_owner_id,
            "originator_name": settings.service_owner_name,
            "saleArea": sale_area,
        "dept": dept_id,
        "dept_name": dept_name,
        "summary": summary,
        "content": content,
            ...
            "ower": settings.service_owner_id,
            "ower_name": settings.service_owner_name,
            "systemSource": settings.system_source,
            # 更換濾芯任務不寫金額
            "taskDefineCharacter": {},
            "taskExecutorList": [],
